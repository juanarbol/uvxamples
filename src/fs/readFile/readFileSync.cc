...
int main() {
  ...
  // With the mode "S_IRUSR" which means 'Read permission for the owner' or
  // '0400' And pass NULL for the callback, this means that this operation will
  // be syncronus Save uv_fs_open() return value in r, when r < 0, means and
  // error ocurred In case of error, r will be the error code
  ...
  if (r < 0) {
    ...
    // See docs:
    // http://docs.libuv.org/en/v1.x/errors.html?highlight=uv_strerror#c.uv_strerror
  ...
  // size of that buffer will be BUF_SIZE, so, libuv will initialize our buffer
  // in our variable with the right size See docs:
  // http://docs.libuv.org/en/v1.x/misc.html?highlight=uv_buf_init#c.uv_buf_init
  char buf[BUF_SIZE];
  ...
  // Read API docs before continuing reading this code
  // See docs:
  // http://docs.libuv.org/en/v1.x/fs.html?highlight=uv_fs_read#c.uv_fs_read
  ...
  // The offset, which specifies the file offset at which the input operation is
  // to be performed (in this case) is 0, Send NULL to callback param, this is a
  // sync operation uv_file docs:
  // http://docs.libuv.org/en/v1.x/misc.html#c.uv_file

  // Arguments explanation:
  // loop: our main event loop
  // &read_req: memory address where libuv will save the value of reading
  // operation open_req.result: this is actually the file that we got from
  // uv_fs_open &iov: memory address of our declared buffer 1: the number of
  // unsigned buffers 0: means the offset (see line 88) NULL: this is the
  // callback param, send NULL, to make this operation sync
  ...
  // At this time, buf variable will be the actual content of our file, just
  // print it
  printf("%s", buf);
  ...
  // Arguments explanation:
  // loop: our main event loop
  // &close_req: memory address where libuv will save the value of closing file
  // operation open_req.result: this is actually the file that we got from
  // uv_fs_open NULL: this is the callback param, send NULL, to make this
  // operation sync
